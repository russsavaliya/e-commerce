  ...
  const Pagination = () => {
    ...
    return (
      <div className="flex items-center justify-center gap-2 mt-8">
        ...
        >
          <ChevronLeft className="w-5 h-5 text-[#481d6f]" />
        </button>
        {[...Array(totalPages)].map((_, index) => {
          ...
          if (
            ...
            return (
              <button
                ...
                className={`px-4 py-2 border rounded-lg font-medium transition-colors ${currentPage === page
                  ? 'bg-[#481d6f] text-white border-[#481d6f]'
                  : 'border-gray-300 hover:bg-[#faf9f5] text-[#481d6f]'
                  }`}
              >
                {page}
              </button>
            ...
            return <span key={page} className="px-2 text-[#481d6f]">...</span>;
          }
          return null;
        })}
        <button
          ...
          className="p-2 border border-gray-300 rounded-lg hover:bg-[#faf9f5] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          <ChevronRight className="w-5 h-5 text-[#481d6f]" />
        </button>
      </div>
    );
  };
    ...
    <div className="min-h-screen bg-[#faf9f5]">
      ...
      <div className="lg:hidden sticky top-20 z-40 bg-[#faf9f5] border-b border-gray-200 shadow-sm px-4 py-3">
        <button
          ...
          className="flex items-center gap-2 text-sm font-semibold text-[#481d6f] uppercase tracking-wide hover:text-[#481d6f] transition-colors"
        >
          <SlidersHorizontal className="w-5 h-5 text-[#481d6f]" />
          Filters
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-80 bg-[#faf9f5] shadow-xl overflow-y-auto">
            <div className="sticky top-0 bg-[#faf9f5] border-b border-gray-200 p-4 flex items-center justify-between z-10">
              <h3 className="text-lg font-semibold text-[#481d6f] uppercase">Filters</h3>
              <button
                onClick={() => setMobileFiltersOpen(false)}
          ...
          <div className="flex-1 min-w-0">
            ...
            {loading ? (
              <div className="flex items-center justify-center py-16 bg-[#faf9f5]">
                <Loader2 className="w-8 h-8 animate-spin text-[#481d6f]" />
              </div>
            ) : products.length === 0 ? (
              <div className="py-16 text-center bg-[#faf9f5]">
                <p className="text-[#481d6f] mb-4">No products found matching your filters.</p>
                <button
                  onClick={clearFilters}
                  className="text-[#481d6f] hover:opacity-80 font-medium uppercase tracking-wide transition-colors"
                >
                  Clear Filters
                </button>
              </div>
